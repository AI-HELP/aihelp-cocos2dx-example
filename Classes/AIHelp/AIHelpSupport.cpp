...
OnAIHelpInitializedCallback initAsyncCallBack = nullptr;
...
JNIEXPORT void JNICALL Java_net_aihelp_init_CallbackHelper_handleCocos2dxCallback
    ...
    if (type == 1000 || type == 1001) {
        ...
        if (objArray != nullptr && jniEnv->GetArrayLength(objArray) >= 2) {
            jobject successObj = jniEnv->GetObjectArrayElement(objArray, 0);
            jobject messageObj = jniEnv->GetObjectArrayElement(objArray, 1);

            if (jniEnv->IsInstanceOf(successObj, jniEnv->FindClass("java/lang/Boolean"))) {
                isSuccess = jniEnv->CallBooleanMethod(successObj,
                                                      jniEnv->GetMethodID(jniEnv->FindClass("java/lang/Boolean"),
                                                                          "booleanValue", "()Z"));
            }

            if (jniEnv->IsInstanceOf(messageObj, jniEnv->FindClass("java/lang/String"))) {
            ...
            if (message != nullptr) {
                jniEnv->ReleaseStringUTFChars(static_cast<jstring>(messageObj), message);
            }
        }
